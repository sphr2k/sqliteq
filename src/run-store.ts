...
function parseJson<T>(value: null | string, fallback: T): T {
  if (!value) {
    return fallback
  }
...
function applyDeltas(current: CounterMap, deltas: CounterMap | undefined): CounterMap {
  if (!deltas) {
    return current
  }
...
export class RunStore {
  ...
  async getRunSnapshot(runId: string): Promise<RunSnapshot | null> {
    ...
    return {
      ...
      queue: row.queue,
      status: row.status,
      scope: parseJson<JsonRecord | null>(row.scope_json, null),
      counters: parseJson<CounterMap>(row.counters_json, {}),
  ...
  async record(input: RecordRunUpdateInput): Promise<boolean> {
  ...
  async pruneExpiredRuns(now: Date): Promise<{ events: number; runs: number }> {
    const expiredRunsResult = await this.driver.execute(SQL_SELECT_EXPIRED_RUNS, [
      now.toISOString(),
    ])
    const runIds = expiredRunsResult.rows.map((row) => row.run_id as string)

    if (runIds.length === 0) {
      return { runs: 0, events: 0 }
    }

    const eventCountResult = await this.driver.execute(
      SQL_COUNT_EVENTS_FOR_RUNS.replace('%RUN_IDS%', placeholders(runIds.length)),
      runIds,
    )
    const events = Number(eventCountResult.rows[0]?.count ?? 0)

    const deleteResult = await this.driver.execute(
      SQL_DELETE_RUNS.replace('%RUN_IDS%', placeholders(runIds.length)),
      runIds,
    )
