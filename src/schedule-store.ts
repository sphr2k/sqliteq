...
function rowToWindow(row: Record<string, any>): ScheduleWindow {
  return {
    scheduleKey: row.schedule_key,
    windowKey: row.window_key,
    status: row.status as ScheduleWindowStatus,
...
export class ScheduleStore {
  ...
  async claimWindow(
    scheduleKey: string,
    windowKey: string,
    options?: ClaimWindowOptions,
  ): Promise<boolean> {
    if (!scheduleKey) throw new Error('scheduleKey is required')
   ...
   */
  async getWindow(scheduleKey: string, windowKey: string): Promise<ScheduleWindow | null> {
   ...
   */
  async listWindows(scheduleKey: string, limit = 50): Promise<ScheduleWindow[]> {
    if (!scheduleKey) throw new Error('scheduleKey is required')
   ...
   */
  async markWindowFinished(
    scheduleKey: string,
    windowKey: string,
    status: 'completed' | 'failed',
    options?: MarkWindowFinishedOptions,
  ): Promise<boolean> {
    const now = toIso(new Date())
